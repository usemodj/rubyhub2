...
module HabtmCheckboxes
  ...
  #   <% end -%>
  def habtm_checkboxes(obj, column, assignment_objects, assignment_object_display_column, checkbox_options={})
    ...
    assignment_objects.each do |assignment_obj|
      ...
      checkbox_options.merge( :id => cbx_id)
      #check_box_tag(name, value = "1", checked = false, options = {})
      html += check_box_tag field_name,  assignment_obj.id,  obj.send(column).include?(assignment_obj.id), checkbox_options
